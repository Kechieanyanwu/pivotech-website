    ...
    <>
      {stats.map((stat) => (
        <div key={stat.label} className="shrink-0">
          <b className="font-bold">{stat.number}</b>{" "}
          <span className="text-beige/70">{stat.label}</span>
        </div>
      ))}
    ...
    <div className="bg-dark-navy text-beige font-sans text-[15px]">
      <div className="overflow-hidden py-5 md:hidden">
        ...
        <div className="stats-ribbon__track" aria-hidden="true">
          ...
          <div className="stats-ribbon__group stats-ribbon__duplicate">
            {items}
          </div>
        </div>
      </div>
      <div className="hidden flex-wrap items-center justify-between gap-x-10 gap-y-3 px-12 py-5 md:flex">
        {items}
      </div>
    </div>
  );
