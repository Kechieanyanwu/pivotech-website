...
import { defaultSettings, type SiteSettings } from "@/content/community";
  ...
  return (
    ...
    >
      ...
      <div className="px-8 md:px-12 py-12 md:border-r border-navy/12">
        ...
        <div className="mt-6 grid gap-4">
          {events.map((event) => (
            <article
              key={event.id}
              className="rounded-xl border border-navy/15 p-5"
            >
              <p className="font-sans text-sm font-semibold uppercase tracking-wider text-blue">
                {event.city}
              </p>
        ...
        <p className="mt-4 font-sans text-[15px] text-navy/70">
          Something to share with the room?{" "}
          <Link
            href="/submit?type=talk"
            className="inline-flex min-h-11 items-center font-semibold text-blue hover:underline"
          >
            Submit a talk →
          </Link>
        </p>
      ...
      <div className="px-8 md:px-12 py-12">
          ...
          {posts.map((post) => (
            ...
            >
              ...
              <time
                ...
                className="mt-2 block font-sans text-sm text-navy/60"
              >
        ...
        {posts.length === 0 && (
          <p className="mt-4 font-sans text-navy/65">
            Explore our latest stories and conversations on Substack.
          </p>
        )}
        <a
